...
app.post('/api/verifyLogin', async (req, res) => {
  try {
    ...
    if (role === 'employee') {
      ...
      if (empIdx !== -1 && (pass === '1234' || pass === safeStr(empRows[empIdx][3]))) {
        return res.json({ 
          success: true, 
...
app.get('/api/dashboard', async (req, res) => {
  try {
    ...
    const totalStudents = sRows.length - 1;
    const activeStudents = sRows.slice(1).filter(r => safeStr(r[11]) === 'نشط').length;
    const totalCourses = cRows.length - 1;
    const totalTeachers = tRows.length - 1;
